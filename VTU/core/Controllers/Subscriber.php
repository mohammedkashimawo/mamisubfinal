       ...
       $transrer = generatebulkref();
			...
			if(is_object($check)){
				...
				if($result->status == "success"){
					header("Location: sendbulksms?ref=$transref");
					return 0;
				}
				else{
...
//-----------------------------—---------------------
    ...
    return $transref;
       ...
       $transre = generateTransref();
		...
		public function getAllDataTransaction($limit){
			$check=$this->model->getAllDataTransaction($this->userId,$limit);
			return $check;
		}
